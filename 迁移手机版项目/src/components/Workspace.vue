...
<template>
  ...
  >
    ...
    <main class="flex-1 flex flex-col min-w-0">
      ...
      <nav
        class="fixed inset-x-0 bottom-0 z-40 flex h-[4.5rem] items-center justify-around border-t border-gray-200 bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_18px_rgba(20,20,19,0.08)] backdrop-blur md:hidden dark:border-[#2e2e2c] dark:bg-[#1e1e1c]/95"
        aria-label="移动端导航"
      >
        <button
          class="mobile-nav-item"
          :class="mode === 'edit' ? 'mobile-nav-active' : ''"
          @click="emit('exit-review')"
        >
          <span class="text-lg">⌂</span><span>错题本</span>
        </button>
        <button
          class="mobile-nav-item"
          :class="mode === 'review' ? 'mobile-nav-active' : ''"
          @click="emit('start-review', false)"
        >
          <span class="text-lg">✓</span><span>今日复习</span>
        </button>
        <button class="mobile-nav-item" @click="emit('open-plugin', 'english-vocabulary')">
          <span class="text-lg">Aa</span><span>语言学习</span>
        </button>
        <button class="mobile-nav-item" @click="emit('toggle-settings')">
          <span class="text-lg">⚙</span><span>设置</span>
        </button>
      </nav>
